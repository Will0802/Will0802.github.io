...
function renderingCategory(categoryArray) {
  ...
  categoryArray.forEach((item) => {
    ...
    // button.onclick = () => {
    //   fetchMerchandise(item.dataUrl).then((data) => {
    //     console.log(data);
    //   });
    // };
...
function createTitleArea(title, price) {
  ...
  titleArea.innerHTML = `<h1>購買 ${title}</h1>
  <div class="total-price">NT$ ${price.toLocaleString()} 起</div>`;
}
...
function createCarouselIndicatorsHTML(images) {
  ...
  images.forEach((img, index) => {
    html += `<button type="button" data-bs-target="#carouselExampleAutoplaying" data-bs-slide-to="${index}"
...
function createWidgetHTML(widget) {
  ...
  items.forEach((item) => {
    ...
    <div
      class="border border-secondary-subtle rounded-3 p-4 text-center"
      role="button" onclick="clickHandler(this,'${widget.type}')" data-color="${color.colorCode}"
    >
      <img
        class="w-25"
        src="${color.colorImg}"
      ...
      const spec = shopData.specifications.filter(
        (spec) => spec.model === item
      );
      const minPrice = Math.min(...spec.map((s) => s.price));
      itemHTML += `<div class="col">
      <div
        class="border border-secondary-subtle rounded-3 p-4"
        role="button" onclick="clickHandler(this,'${widget.type}')" data-model="${item}"
      >
        ${item}
      </div>
    </div>`;
      ...
      itemHTML += ` <div class="col">
        <div
          class="border border-secondary-subtle rounded-3 p-4 d-flex justify-content-between"
          role="button" onclick="clickHandler(this,'${widget.type}')" data-storage="${item}"
        >
          <div class="storage-spec">${item}</div>
          <div class="price">NT$ 34,900</div>
        </div>
      </div>`;
    } else {
      itemHTML += ` <div class="col">
    <div
      class="border border-secondary-subtle rounded-3 p-4"
      role="button" onclick="clickHandler(this,'${widget.type}')"
    >
      <div class="">${item}</div>
    </div>
  </div>`;
  ...
  let html = `
    <section class="widget-item mb-4 mx-lg-3">
        <h2 class="fs-4">${widget.title} <span class="text-black-50">${
    widget.subTitle
  }</span></h2>
        ${
          widget.type === "color"
            ? `<p><span class="picked-color fw-medium">顏色</span> </p>`
            : ""
        }
...
function clickHandler(element, type) {
  ...
  if (type === "color") {
    ...
    const color = shopData.colors.find(
      (c) => c.colorCode === element.dataset.color
    );
